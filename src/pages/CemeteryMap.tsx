...
const DEFAULT_STATUS = { label: '未知', cellBg: 'bg-gray-100', cellBorder: 'border-gray-300', badgeClass: 'bg-gray-100 text-gray-600' }

const TYPE_LABELS: Record<string, string> = { single: '单穴', double: '双穴', family: '家族' }
const STATUS_FILTERS: { key: PlotStatus | 'all'; label: string }[] = [
  { key: 'all', label: '全部' }, { key: 'available', label: '空闲' }, { key: 'reserved', label: '预留' },
  { key: 'sold', label: '已售' }, { key: 'buried', label: '安葬' }, { key: 'maintenance', label: '维护' },
]
...
export default function CemeteryMap() {
  ...
  const filteredPlots = useMemo(() => {
    return plots.filter((p) => {
      ...
      if (searchText && !p.position.includes(searchText) && !p.holderName?.includes(searchText) && !p.deceasedName?.includes(searchText)) return false
      return true
    })
    ...
    <div className="min-h-screen bg-cream p-4 flex flex-col gap-4">
      ...
      <div className="card p-4 flex items-center gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 ml-auto flex-wrap">
          ...
          <select className="select-field w-32" value={selectedArea}
          ...
          <div className="relative">
            ...
            <input className="input-field pl-8 w-40" placeholder="搜索位置/持有人/逝者"
              value={searchText} onChange={(e) => setSearchText(e.target.value)} />
            {searchText && <X className="w-4 h-4 absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 cursor-pointer" onClick={() => setSearchText('')} />}
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <div className="w-56 shrink-0 flex flex-col gap-2 overflow-y-auto">
          <h2 className="section-title text-sm flex items-center gap-1"><Info className="w-4 h-4" /> 园区列表</h2>
          {areas.map((a) => {
            const pct = Math.round((a.soldPlots / a.totalPlots) * 100)
            return (
              <div key={a.id} onClick={() => { setSelectedArea(a.id); setSelectedPlot(null) }}
                ...
                <div className="flex justify-between text-xs mt-2 text-charcoal">
                  <span>总计 {a.totalPlots}</span><span>已售 {a.soldPlots}</span>
                </div>
        ...
        <div className="card p-4 flex-1 overflow-auto">
          {currentArea ? (
            ...
            <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-sage">
              <Map className="w-10 h-10 mb-3 opacity-30" />
              <p className="text-sm font-medium mb-4">请选择具体园区查看墓位分布图</p>
              <div className="grid grid-cols-2 gap-3 w-full max-w-md">{areas.map((a) => (
                <div key={a.id} onClick={() => { setSelectedArea(a.id); setSelectedPlot(null) }}
                  className="card p-3 cursor-pointer hover:ring-2 hover:ring-gold transition-all text-center">
                  <div className="font-serif font-semibold text-primary text-sm">{a.name}</div>
                  <div className="text-xs mt-1">总计 {a.totalPlots} · 已售 {a.soldPlots}</div>
                </div>
              ))}</div>
            </div>
          )}
          ...
          {activePlot ? (
            <div className="card p-4 flex flex-col gap-3">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-sage">位置</span><span className="font-medium">{activePlot.position}</span></div>
                <div className="flex justify-between items-center">
                  <span className="text-sage">状态</span>
                  <span className={(STATUS_CONFIG[activePlot.status] ?? DEFAULT_STATUS).badgeClass}>{(STATUS_CONFIG[activePlot.status] ?? DEFAULT_STATUS).label}</span>
                </div>
                <div className="flex justify-between"><span className="text-sage">类型</span><span>{TYPE_LABELS[activePlot.type] || activePlot.type}</span></div>
                <div className="flex justify-between"><span className="text-sage">价格</span><span className="text-gold-dark font-semibold">¥{activePlot.price.toLocaleString()}</span></div>
                <div className="flex justify-between"><span className="text-sage">朝向</span><span>{activePlot.orientation}</span></div>
                <div className="flex justify-between"><span className="text-sage">面积</span><span>{activePlot.area}㎡</span></div>
              </div>
              {activePlot.holderName && (
                <div className="border-t border-border pt-2 space-y-2 text-sm">
                  ...
                  <div className="flex justify-between"><span className="text-sage">持有人</span><span>{activePlot.holderName}</span></div>
                  {activePlot.deceasedName && <div className="flex justify-between"><span className="text-sage">逝者</span><span>{activePlot.deceasedName}</span></div>}
                </div>
              )}
            </div>
          ) : (
            <div className="card p-4 flex flex-col items-center justify-center text-sage text-sm h-full min-h-[200px]">
              <Info className="w-8 h-8 mb-2 opacity-40" /><span>点击墓位查看详情</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="card p-3 flex items-center gap-6 justify-center">
        {(Object.entries(STATUS_CONFIG) as [PlotStatus, typeof STATUS_CONFIG[PlotStatus]][]).map(([key, cfg]) => (
          <div key={key} className="flex items-center gap-1.5 text-xs text-charcoal">
            <div className={`w-4 h-3 rounded-sm border ${cfg.cellBg} ${cfg.cellBorder}`} /><span>{cfg.label}</span>
          </div>
        ))}
      </div>
    </div>
