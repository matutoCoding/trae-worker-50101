...
const TASKS = [
  { id: 1, text: '陈老先生安葬仪式 — 兰亭园1排1号', time: '08:00', urgency: 'red' as const },
  { id: 2, text: '祭扫接待：张明远家属 — 松柏园1排1号', time: '08:00', urgency: 'yellow' as const },
  { id: 3, text: '管理费催缴提醒 — 李文华(逾期)', time: '10:00', urgency: 'red' as const },
  { id: 4, text: '梅花园墓碑清洁及道路清扫', time: '14:00', urgency: 'green' as const },
  { id: 5, text: '祭扫预约确认 — 黄志强代客祭扫', time: '14:00', urgency: 'yellow' as const },
  { id: 6, text: '兰亭园3排2号墓碑底座维修', time: '16:00', urgency: 'yellow' as const },
]

const URGENCY_DOT = { red: 'bg-red-500', yellow: 'bg-amber-400', green: 'bg-emerald-400' }

export default function Dashboard() {
  const { plots, contracts } = useStore()
  ...
  const stats = useMemo(() => {
    ...
    const maintenance = plots.filter((p) => p.status === 'maintenance').length
    return {
      total: plots.length,
      ...
      pieData: [
        ...
        { name: '维护中', value: maintenance, key: 'maintenance' },
      ].filter((d) => d.value > 0),
    }
  }, [plots])
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={monthlySales} barSize={36}>
            ...
            <Tooltip
              ...
              contentStyle={{ borderRadius: 8, border: '1px solid #E2DDD5', fontSize: 13 }}
            />
            <Bar dataKey="count" fill="#1B3A2D" radius={[4, 4, 0, 0]}
              onMouseEnter={(e) => { e.fill = '#C4A35A' }}
              onMouseLeave={(e) => { e.fill = '#1B3A2D' }}
            />
          </BarChart>
        </ResponsiveContainer>
      </div>
